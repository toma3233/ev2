...
echo "Grant release tracker read permission to toggle storage account"

RELEASETRACKER_USER_ASSIGNED_IDENTITY_NAME="${RESOURCE_NAME_PREFIX}releasetracker"
OVERLAY_RESOURCE_GROUP_NAME="${OVERLAY_RESOURCE_GROUP_NAME:-"${RESOURCE_NAME_PREFIX}overlay-${REGION}"}"

ASSIGNEE=$(az identity show --name ${RELEASETRACKER_USER_ASSIGNED_IDENTITY_NAME}    \
