...
echo "Grant rl read permission to toggle storage account"

RL_USER_ASSIGNED_IDENTITY_NAME="${RESOURCE_NAME_PREFIX}aks-rl"
...
echo "Grant rl read/write permission to dns zone"

az role assignment create --role ${DNS_ZONE_CONTRIBUTOR_ROLE} \
                          --assignee-object-id ${ASSIGNEE} \
                          --scope ${DNS_ZONE_ID} \
                          || echo 'error updating rbac'
